...
export class NaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dataService.getServerStatsNA().subscribe((data: any[])=>{
      this.server_stats = data;
      this.server_stats = this.server_stats.data[0];
    })  

    //this.dataService.getFFAwlRatioNA().subscribe((data: any[])=>{
    //  this.ffa_wl_ratio = data;
    //  this.ffa_wl_ratio = this.ffa_wl_ratio.data;
    //})  

    this.dataService.getSaberEloLevelNA().subscribe((data: any[])=>{
      this.saber_elo_level = data;
      this.saber_elo_level = this.saber_elo_level.data;
    })  

    this.dataService.getFFEloLevelNA().subscribe((data: any[])=>{
